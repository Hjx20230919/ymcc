...
public class BizSettlementController extends BaseController{
    ...
    @Autowired
    private BizReceiptpaymentService bizReceiptpaymentService;
    ...
    @ApiOperation("新增收付款")
    @Log(logContent = "新增收付款",logModule = LogModule.SETTLEMENT,logType = LogType.OPERATION)
    @RequestMapping(value = "/receiptpaymen", method = RequestMethod.POST)
    public AppMessage addsfk(@RequestBody RreceiptVo rreceiptVo)throws Exception {
        BizReceiptpaymentDto bizReceiptpaymentDto = new BizReceiptpaymentDto();
        bizReceiptpaymentDto.setDealId(rreceiptVo.getDealId());
        bizReceiptpaymentDto.setReceiptTime(rreceiptVo.getReceiptTime());
        bizReceiptpaymentDto.setSettlementId(rreceiptVo.getSettlementId());
        bizReceiptpaymentDto.setReceiptAmount(rreceiptVo.getReceiptAmount());
        bizReceiptpaymentDto.setModepayment(rreceiptVo.getModepayment());
        bizReceiptpaymentService.save(bizReceiptpaymentDto);
//        settlementService.up
        BizSettlementDto bizSettlementDto = settlementService.selectByKey(rreceiptVo.getSettlementId());
        double v =0;
        if (bizSettlementDto.getActualPayAmount()==null){
            v = Double.valueOf(bizReceiptpaymentDto.getReceiptAmount());
        }else {
            v = bizSettlementDto.getActualPayAmount() + Double.valueOf(bizReceiptpaymentDto.getReceiptAmount());
        }


        bizSettlementDto.setActualPayAmount(v);
        settlementService.updateAll(bizSettlementDto);
        return AppMessage.success(bizReceiptpaymentDto.getSettlementId(), "新增收付款成功");
    }
    @ApiOperation("查询收付款")
    @RequestMapping(value = "/getreceiptpaymen",method = RequestMethod.GET)
    public AppMessage receiptquery(@RequestParam(value = "pageNum",defaultValue = "1") int pageNo,
                            @RequestParam(value = "pageSize",defaultValue = "20") int pageSize,
                            @RequestParam(value = "id",defaultValue="") String settleId,
                            @RequestParam(value = "dealId",defaultValue="") String dealId) {

        Map<String, Object> params = new HashMap<String, Object>();
        params.put("settlementId", settleId);
//        params.put("dealId", dealId);
        BizSettlementDto bizSettlementDto = settlementService.selectByKey(settleId);
        //1、设置分页信息，包括当前页数和每页显示的总计数
        PageHelper.startPage(pageNo, pageSize);
        //2、执行查询
        List<BizReceiptpaymentDto> bizReceiptpaymentDtos = bizReceiptpaymentService.selectList(params);
        for (BizReceiptpaymentDto bizReceiptpaymentDto : bizReceiptpaymentDtos) {
//            bizReceiptpaymentDto.setModepayment(bizSettlementDto.getMilestone());
            bizReceiptpaymentDto.setCreateTime(bizSettlementDto.getCreateTime());
            bizReceiptpaymentDto.setMilestone(bizSettlementDto.getMilestone());
        }

        //3、获取分页查询后的数据
        TableDataInfo dataTable = getDataTable(bizReceiptpaymentDtos);
        return AppMessage.success(dataTable, "查询收付款成功");
    }
    @ApiOperation("修改收付款")
    @RequestMapping(value = "/putreceiptpaymen",method = RequestMethod.PUT)
    public AppMessage putreceiptquery(@RequestBody RreceiptVo rreceiptVo) {
        BizReceiptpaymentDto bizReceiptpaymentDto = bizReceiptpaymentService.selectByKey(rreceiptVo.getId());
        String receiptAmount = bizReceiptpaymentDto.getReceiptAmount();
        Double aDouble = Double.valueOf(receiptAmount);
        Double receiptAmount1 = Double.valueOf(rreceiptVo.getReceiptAmount());

        String settlementId = bizReceiptpaymentDto.getSettlementId();
        BizSettlementDto bizSettlementDto = settlementService.selectByKey(settlementId);
        if (aDouble>receiptAmount1){

            bizSettlementDto.setActualPayAmount(bizSettlementDto.getActualPayAmount()+aDouble-receiptAmount1);
        }else if (aDouble<receiptAmount1){
            bizSettlementDto.setActualPayAmount(bizSettlementDto.getActualPayAmount()+receiptAmount1-aDouble);
        }else {
            bizSettlementDto.setActualPayAmount(bizSettlementDto.getActualPayAmount());
        ...
        bizReceiptpaymentService.updateAll(bizReceiptpaymentDto);
        return AppMessage.success(bizReceiptpaymentDto.getSettlementId(), "修改收付款成功");

    }
    @ApiOperation("删除收付款")
    @RequestMapping(value = "delttereceiptpaymen/{id}",method = RequestMethod.DELETE)
    public AppMessage deletereceiptquery(@PathVariable String id) {
        BizReceiptpaymentDto bizReceiptpaymentDto = bizReceiptpaymentService.selectByKey(Integer.valueOf(id));
        String receiptAmount = bizReceiptpaymentDto.getReceiptAmount();
        String settlementId = bizReceiptpaymentDto.getSettlementId();
        BizSettlementDto bizSettlementDto = settlementService.selectByKey(settlementId);
        bizSettlementDto.setActualPayAmount(bizSettlementDto.getActualPayAmount()-Double.valueOf(bizReceiptpaymentDto.getReceiptAmount()));
        settlementService.updateAll(bizSettlementDto);
        bizReceiptpaymentService.delete(Integer.valueOf(id));
        return AppMessage.success( id, "删除收付款成功");
    ...
    public AppMessage add(@RequestBody SettleParamVo settleParamVo)throws Exception {
        ...
        if("收入".equals(bizDealDto.getDealIncome())||settlementService.isAvailableAmount(settlementVo.getSettleAmount(),settlementVo.getDealId())){
            ...
            settlementDto.setMilestone(settlementVo.getMilestone());
