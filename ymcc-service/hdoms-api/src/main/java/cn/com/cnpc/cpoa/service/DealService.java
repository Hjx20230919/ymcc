...
private  String RECYCLEBINURL;
    ...
    public boolean updateChain(BizDealDto dealDto, List<BizAttachDto> attachDtos, List<BizDealAttachDto> dealAttachDtos, String type, String userId,String status)throws Exception{
        ...
        if (dealDto.getTaxRate()==null){
            dealDto.setTaxRate(0);
        }
        //3 修改合同
...
//        }
    ...
    public String getDealNo(String dealType){
        ...
        return dealType+"-hd-"+ year+"-"+String.format("%04d", (Integer.valueOf(dealNo4)+1));
    }
    ...
    public List<BizDealDto> selectuserById(Map<String,Object> map){
        List<BizDealDto> list = bizDealDtoMapper.selectuserById(map);
        return list;
    }
...
//        String userId1 = sysUserDto.getUserId();
        List<BizDealDto> bizDealDtoList = dealService.selectuserById(map);
        ...
        for (BizDealDto dealDto : bizDealDtoList) {
            if (dealDto.getDealEnd()==null){
                continue;
            }
            if (dealDto.getDealEnd().before(threeMonthsLater)) {
                SimpleDateFormat simpleDateFormat = new SimpleDateFormat();
                String format = simpleDateFormat.format(dealDto.getDealEnd());
                dealDto.setInformation("合同将于"+format+"过期请及时处理");
