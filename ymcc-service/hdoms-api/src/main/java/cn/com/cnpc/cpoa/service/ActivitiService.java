...
//            String deptManager2 = sysDeptDto2.getDeptManager();
            ...
            if (StringUtils.isNotEmpty(deptManager) && !settleFlag) {
                ...
                stepSplits=checkStepDto1.getStepId();
...
//            checkManDto1.setCheckTime(DateUtils.getNowDate());
            ...
            if (!settleFlag) {
                if (deptManager2.equals(deptManager)){
                    BizCheckStepDto bizCheckStepDto = checkStepService.selectByKey(stepSplits);
                    bizCheckStepDto.setStepSplit(1);
                    checkStepService.updateAll(bizCheckStepDto);
                }else {
                    // 7 新建第二步审核
...
//        checkStepDto.setStepEndAt(nowDate);
    ...
    public void saveProInitMap(Map<String, Object> initMap, int stepNo) {
        List<BizCheckStepDto> checkStepDtos = new ArrayList<>();//发起
...
//        Boolean b=false;
        for (int i = 0; i < stepNo; i++) {
            ...
            Iterator<BizCheckStepDto> stepIterator = checkStepDtos.iterator();
            while (stepIterator.hasNext()) {
                BizCheckStepDto stepDto = stepIterator.next();

                if (!stepDto.getStepName().equals("发起") && !checkStepDto.getStepName().equals("发起")){
//                    String stepId = checkStepDto.getStepId();
                    String stepId1 = stepDto.getStepId();
//                    BizCheckManDto filteredList = checkManDtos.stream()
//                            .filter(manDto -> manDto.getStepId().equals(stepId))
//                            .collect(Collectors.toList()).get(0);

                    BizCheckManDto filteredList1 = checkManDtos.stream()
                            .filter(manDto -> manDto.getStepId().equals(stepId1))
                            .collect(Collectors.toList()).get(0);
                    if (checkManDto.getUserId().equals(filteredList1.getUserId())){
                        stepIterator.remove();  // 使用迭代器进行安全删除
                        Iterator<BizCheckManDto> manIterator = checkManDtos.iterator();
                        while (manIterator.hasNext()) {
                            BizCheckManDto manDto = manIterator.next();
                            if (manDto.getStepId().equals(stepDto.getStepId())) {
                                manIterator.remove();  // 使用迭代器进行安全删除
                                break;
                            }
                        }
                    }
                ...
                if (checkStepDto.getStepName().equals("部门审核")&&!stepDto.getStepName().equals("发起")) {
                    if (stepDto.getStepName().equals("企管审核") && checkStepDto.getStepName().equals("部门审核")){
                        String stepId = checkStepDto.getStepId();
                        String stepId1 = stepDto.getStepId();
                        BizCheckManDto filteredList = checkManDtos.stream()
                                .filter(manDto -> manDto.getStepId().equals(stepId))
                        ...
                        BizCheckManDto filteredList1 = checkManDtos.stream()
                                .filter(manDto -> manDto.getStepId().equals(stepId1))
                                .collect(Collectors.toList()).get(0);
                        if (filteredList.getUserId().equals(filteredList1.getUserId())){
                            stepIterator.remove();  // 使用迭代器进行安全删除
                            Iterator<BizCheckManDto> manIterator = checkManDtos.iterator();
                            while (manIterator.hasNext()) {
                                BizCheckManDto manDto = manIterator.next();
...
//        Iterator<BizCheckStepDto> iterator = checkStepDtos.iterator();
//        while (iterator.hasNext()) {
//            BizCheckStepDto checkStepDto = iterator.next();
//            for (BizCheckStepDto stepDto : checkStepDtos) {
//                if (stepDto.getStepName().equals("企管审核") && checkStepDto.getStepName().equals("部门审核") && checkStepDto.getStepCreateUser().equals(stepDto.getStepCreateUser())) {
//                    iterator.remove();
//                    break; // 如果找到匹配的项，跳出内层循环
//                }
//            }
//        }
        // 根据 stepNo 从小到大排序
        List<BizCheckStepDto> sortedList = checkStepDtos.stream()
                .sorted(Comparator.comparingInt(BizCheckStepDto::getStepNo))
                .collect(Collectors.toList());

        // 重新赋值 stepNo 从 0 开始
        int newStepNo = 0;
        for (BizCheckStepDto stepDto : sortedList) {
            stepDto.setStepNo(newStepNo++);
        }
