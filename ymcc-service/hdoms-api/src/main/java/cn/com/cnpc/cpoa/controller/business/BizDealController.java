...
public class BizDealController extends BaseController {
    ...
    @Autowired
    BizReceiptpaymentService bizReceiptpaymentService;
...
//
    ...
    public AppMessage exclude( @RequestBody DealParamVo dealexVo) throws Exception {
        String userId = ServletUtils.getSessionUserId();
        List<AttachVo> attachVos = dealexVo.getAttachVos();
...
//        bizDealExService.save(bizDealExDto);
        ...
        if (!bizDealExService.saveChain(bizDealExDto, attachDtos, dealAttachDtos, type)) {
...
//        }
    ...
    public AppMessage grandsave( @RequestBody DealParamVo dealHdVo) throws Exception {
        ...
        bizDealHdDto.setDealNo(dealHd.getDealNo());
        ...
        bizDealHdDto.setDeptId(dealHd.getDeptId());
        ...
        bizDealHdDto.setUserId(dealHd.getUserId());
...
//2 附件
        ...
        if (!bizDealHdService.saveChain(bizDealHdDto, attachDtos, dealAttachDtos, type)) {
            return AppMessage.error("新增合同出错");
        }
...
//            params.put("dataScopeAll", "dataScopeAll");
        ...
        for (BizDealDto sysDealDto : sysDealDtos) {
            Map<String, Object> objectObjectHashMap = new HashMap<>();
            String dealId1 = sysDealDto.getDealId();
            objectObjectHashMap.put("dealId",dealId1);

            List<BizReceiptpaymentDto> bizReceiptpaymentDtos = bizReceiptpaymentService.selectBuDealId(objectObjectHashMap);
            Double totalReceiptAmount = bizReceiptpaymentDtos.stream()
                    .mapToDouble(dto -> {
                        try {
                            return Double.parseDouble(dto.getReceiptAmount());
                        } catch (NumberFormatException e) {
                            return 0.0;
                        }
                    })
                    .sum();
            sysDealDto.setReceiptAmount(totalReceiptAmount.toString());
        }
        //3、获取分页查询后的数据
...
//        List<DealImportVo> dealVo = util.importExcel(file.getInputStream());
//        return importLogService.saveDealDtos(dealVo);
        ExcelUtil<DealimportElseVo> util = new ExcelUtil<DealimportElseVo>(DealimportElseVo.class);
        String filename = file.getOriginalFilename();
        List<DealimportElseVo> dealVo = util.importExcel2(filename,file.getInputStream());
        List<DealimportElseVo> filteredList = dealVo.stream()
                .filter(deal -> deal.getDealName() != null && !deal.getDealName().isEmpty())
                .collect(Collectors.toList());
        dealVo.forEach(p->{
            p.setCreateAt(p.getDealSignTime());
        });
        dealVo.clear();
        dealVo.addAll(filteredList);
     ...
     */
    @Log(logContent = "集团合同上传", logModule = LogModule.DEAL, logType = LogType.OPERATION)
    @RequestMapping(value = "/templateupload", method = RequestMethod.GET)
    public void exportdeal( HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            response.reset();
            // 定义浏览器响应表头，并定义下载名
            String fileName = URLEncoder.encode("合同模板.xlsx", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + fileName);
            //定义下载的类型，标明是excel文件
            response.setContentType("application/vnd.ms-excel;charset=UTF-8");
            //把创建好的excel写入到输出流
            ExcelTempUtils.writeTemp(dealTmpl,output);
            //随手关门
            output.close();
        }catch (Exception e){
            e.printStackTrace();
...
//        template.getDocument().write(fos);
     ...
     * 合同拆分
     */
    @RequestMapping(value = "/dealhint",method = RequestMethod.GET)
    ...
    public AppMessage dealTaskMethod()throws Exception{
        List<BizDealDto> bizDealDtoList = dealService.dealTaskMethod();
        return AppMessage.success(bizDealDtoList, "查询合同成功");
    }
}
