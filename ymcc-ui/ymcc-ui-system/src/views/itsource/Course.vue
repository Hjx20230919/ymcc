		...
		<el-dialog title="新增" :visible.sync="addFormVisible"  :close-on-click-modal="false" width="860px">
			<el-form :inline="true" :model="addForm" label-width="80px"  ref="addForm">
				...
				<el-form-item prop="logo" style="width: 400px" >
					...
					<el-upload
							class="upload-demo"
							action="http://lqlymcc.oss-cn-chengdu.aliyuncs.com"
							:data="uploadPicData"
...
<script>
	...
	export default {
		...
		methods: {
			...
			addKillCourseSubmit(){
				this.$http.post("/kill/killCourse/add",this.killCourseForm).then(res=>{
			...
			async getSign(data){
				//让这个请求变成同步请求
				await this.$http.get("/common/file/oss/sign").then(response=>{
					//设置相关的参数
