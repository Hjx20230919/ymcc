<template>
	<section>
      ...
      <el-table-column prop="phone" label="phone"  >
          ...
          <el-button size="small"  @click="handleSave(scope.row)" icon="el-icon-edit" type="primary">拉黑</el-button>
	...
	export default {
		...
		methods: {
			...
			getTableData() {
				...
				this.$http.post("/common/blackList/pagelist",para).then(result=>{
      ...
      //   this.saveFormVisible = true;
      // },
      //9.点击添加和修改对话框的提交按钮提交数据
      saveSubmit: function () {
        //表单验证：只要有一个表单项验证不通过就会返回false，全部通过才会返回true
        this.$refs.saveForm.validate((valid) => {  //ref="saveForm"
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              this.saveLoading = true;

              this.$http.post("/common/blackList/save",this.saveForm).then(res=>{
                if(res.data.success){
                  this.saveFormVisible = false;
                  this.$message.success("成功")
                }else{
                  this.$message.error("系统繁忙【500】")
                }
                this.saveLoading = false;
              }).catch(res=>{
                this.$message.error("系统繁忙【400，404】")
                this.saveLoading = false;
              })
              //关闭对话框
              this.saveFormVisible = false;
